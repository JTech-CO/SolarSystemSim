...
<img width="3814" height="1810" alt="image" src="https://i.imgur.com/try4z3d.png" />
...
- **위성의 실제 공전/자전**: NASA 데이터 기반 위성 궤도 및 자전 주기
- 배경 별 필드 및 태양 글로우 효과
- **시간 가속 기능**: 시각적으로 확인 가능한 속도로 공전/자전 관찰
...
### UI 요소
- **좌측 상단 패널**: 
  - 제어 버튼 (Reset, Top View, Language Toggle)
  - **속도 조절**: 시간 가속 배율 조절 (일시정지, 1x, 10x, 100x, 1000x, 10000x)
  - 현재 속도 표시
...
### 포커싱 및 카메라 제어
- **천체 포커싱**: 행성/위성 선택 시 자동으로 해당 천체에 포커스
- **포커싱 유지**: 포커싱된 천체 주위를 마우스로 회전해도 포커싱 유지
- **자유 카메라**: 빈 공간 클릭 시 포커싱 해제 및 자유 시점 이동
...
│   ├── config.js               # 전역 설정값 (CONFIG 객체, 시간 가속 배율 포함)
│   ├── core/
│   │   ├── scene.js            # Three.js Scene, Camera, Renderer 설정
│   │   ├── controls.js         # OrbitControls, 포커싱 유지, 입력 이벤트 처리
│   │   └── loop.js             # animate 루프, 타원 궤도 업데이트, 자전 처리
│   ├── data/
│   │   ├── planets.js          # 행성 및 위성 데이터셋 (NASA 물리 데이터, 이심률, 위성 공전/자전 주기)
...
│   ├── ui/
│   │   ├── panel.js            # 좌측 정보 패널, 버튼 이벤트, 속도 조절 UI
...
- 위성에도 NASA 데이터 기반 실제 자전 주기 적용
- 시간 가속 배율 적용으로 시각적 관찰 가능

### 시간 가속 시스템 (`js/config.js`, `js/core/loop.js`, `js/ui/panel.js`)
- `timeMultiplier`를 통한 시간 가속 제어
- 공전, 자전, 혜성 이동에 일관된 가속 배율 적용
- UI를 통한 실시간 속도 조절 (1x ~ 10000x)
- 일시정지 기능 지원

### 포커싱 시스템 (`js/core/controls.js`, `js/core/loop.js`)
- OrbitControls 이벤트를 통한 사용자 조작 감지
- 포커싱된 천체 주위 자유 회전 지원
- 천체 이동 시 카메라 자동 추적
...
- **이심률**: NASA 궤도 요소 데이터
- **위성 공전/자전 주기**: NASA 위성 데이터 (조석 고정 포함)
- **표면 중력가속도**: 계산된 값
...
- 시간 가속은 시각적 관찰을 위한 것이며, 물리 계산의 정확성에는 영향을 주지 않습니다.
